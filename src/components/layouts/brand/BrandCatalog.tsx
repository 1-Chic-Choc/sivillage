...
function BrandCatalog({ data }: { data: brandNameType[] }) {
  ...
  // Scroll to specific section based on character
  const scrollToSection = (char: string) => {
    const section = sectionRef.current[char];
    if (section) {
      const targetY = section.offsetTop;
      smoothScrollTo(targetY, 800); // Smooth scroll duration of 800ms
    }
  };
  ...
  // Function to highlight the search keyword in the brand name
